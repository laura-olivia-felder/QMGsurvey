...
qmgBasic::usage="Calculates the lowest eigenstate of the Hamiltonian, the correpsonding eigenvalue and many more omportant quantities like g, omega, the disperion and others at a given point.";


qmgBasicHighPrecision::usage="The same as qmgBasic but to arbitrary precision; Calculates the lowest eigenstate of the Hamiltonian, the correpsonding eigenvalue and many more omportant quantities like g, omega, the disperion and others at a given point.";
...
cqmgBasicINIT::usage="Initialzation function of compiled version of cqmgBasic; cqmgBasic calculates the lowest eigenstate of the Hamiltonian, the correpsonding eigenvalue and many more omportant quantities like g, omega, the disperion and others at a given point.";


cqmgBasicEXTR::usage="Extraction function of compiled version of cqmgBasic; cqmgBasic calculates the lowest eigenstate of the Hamiltonian, the correpsonding eigenvalue and many more omportant quantities like g, omega, the disperion and others at a given point.";
...
Print[Join[Re[pointRaw],Im[pointRaw]]];
If[projectionmode!="slice",((*distinct projection modes*)
...
cqmgTSleafDistributionINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
varB=Table[Xnum[[a]] . Xnum[[b]]-Xnum[[b]] . Xnum[[a]],{a,Dim},{b,Dim}];(*prepare commutators matrix configuration for theta*)
fx3[input_]:=Developer`ToPackedArray[Eigenvectors[input,-1]];(*prepare function that calculates eigensystem of H*)
fx4[input_]:=Developer`ToPackedArray[Orthogonalize[Flatten[{Re[#],Im[#]}&/@Eigenvectors[input,d][[1;;d;;2]],1]]];(*prepare function that calculates orthonormal basis of the distribution for given theta; [[1;;d;;2]] selects the even indices smaller or equal d; for these we calculate the real and imaginary part according to [1] section 3.3.1 hybrid leaf; finally the resulting basis is orthonormalized*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgTSleafDistributionSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgTSleafDistributionSTAT[[2]];(*=d*)
dist=out;
dist
)


cqmgQMleafDistributionINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgQMleafDistributionSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgQMleafDistributionSTAT[[2]];(*=d*)
dist=out;
dist
)


cqmgGQMleafDistributionINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
varA=Sum[Xnum[[a]] . Xnum[[a]],{a,1,Dim}]/2;(*prepare modified square sum over matrix configuration for H*)
fx2[input_]:=Developer`ToPackedArray[ArrayReshape[Eigensystem[input],{Nim+1,Nim}]];(*prepare function that calculates eigensystem of H*)
fx6[input1_,input2_]:=Developer`ToPackedArray[Orthogonalize[Flatten[{Re[#],Im[#]}&/@Eigenvectors[input1,d][[1;;d;;2]],1],Function[{u, v}, u . input2 . v]]];(*prepare function that calculates orthonormal basis of the distribution for given theta; [[1;;d;;2]] selects the even indices smaller or equal d; for these we calculate the real and imaginary part according to [1] section 3.3.1 hybrid leaf; finally the resulting basis is orthonormalized w.r.t. g*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgGQMleafDistributionSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
cqmgDistribution[X_,x_,d_,leaf_:"TSleaf"]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
cqmgDistributionInternal=If[leaf=="TSleaf",(*pick the appropriate function depending on the chosen leaf*)
If[!ValueQ[cqmgTSleafDistributionSTAT],cqmgTSleafDistributionINIT[X,d],If[cqmgTSleafDistributionSTAT!={X,d},cqmgTSleafDistributionINIT[X,d],cqmgTSleafDistribution]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[leaf=="QMleaf",
If[!ValueQ[cqmgQMleafDistributionSTAT],cqmgQMleafDistributionINIT[X,d],If[cqmgQMleafDistributionSTAT!={X,d},cqmgQMleafDistributionINIT[X,d],cqmgQMleafDistribution]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[!ValueQ[cqmgGQMleafDistributionSTAT],cqmgGQMleafDistributionINIT[X,d],If[cqmgGQMleafDistributionSTAT!={X,d},cqmgGQMleafDistributionINIT[X,d],cqmgGQMleafDistribution]](*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
]];
...
cqmgTSleafCurveIterationINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgTSleafCurveIterationSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgTSleafCurveIterationSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgQMleafCurveIterationINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgQMleafCurveIterationSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgQMleafCurveIterationSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgGQMleafCurveIterationINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgGQMleafCurveIterationSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgGQMleafCurveIterationSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgCurveIntegration[X_,x_,v_,delta_,n_,m_,d_,leaf_:"TSleaf",vFix_:False]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
cqmgCurveIteration=If[leaf=="TSleaf",(*pick the appropriate function depending on the chosen leaf*)
If[!ValueQ[cqmgTSleafCurveIterationSTAT],cqmgTSleafCurveIterationINIT[X,d],If[cqmgTSleafCurveIterationSTAT!={X,d},cqmgTSleafCurveIterationINIT[X,d],cqmgTSleafCurveIteration]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[leaf=="QMleaf",
If[!ValueQ[cqmgQMleafCurveIterationSTAT],cqmgQMleafCurveIterationINIT[X,d],If[cqmgQMleafCurveIterationSTAT!={X,d},cqmgQMleafCurveIterationINIT[X,d],cqmgQMleafCurveIteration]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[!ValueQ[cqmgGQMleafCurveIterationSTAT],cqmgGQMleafCurveIterationINIT[X,d],If[cqmgGQMleafCurveIterationSTAT!={X,d},cqmgGQMleafCurveIterationINIT[X,d],cqmgGQMleafCurveIteration]](*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
]];
...
cqmgTSleafCurveIterationNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgTSleafCurveIterationNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgTSleafCurveIterationNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgQMleafCurveIterationNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgQMleafCurveIterationNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgQMleafCurveIterationNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgGQMleafCurveIterationNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgGQMleafCurveIterationNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgGQMleafCurveIterationNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgCurveIntegrationNull[X_,x_,v_,delta_,n_,m_,d_,leaf_:"TSleaf",vFix_:False]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
cqmgCurveIterationNull=If[leaf=="TSleaf",(*pick the appropriate function depending on the chosen leaf*)
If[!ValueQ[cqmgTSleafCurveIterationNullSTAT],cqmgTSleafCurveIterationNullINIT[X,d],If[cqmgTSleafCurveIterationNullSTAT!={X,d},cqmgTSleafCurveIterationNullINIT[X,d],cqmgTSleafCurveIterationNull]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[leaf=="QMleaf",
If[!ValueQ[cqmgQMleafCurveIterationNullSTAT],cqmgQMleafCurveIterationNullINIT[X,d],If[cqmgQMleafCurveIterationNullSTAT!={X,d},cqmgQMleafCurveIterationNullINIT[X,d],cqmgQMleafCurveIterationNull]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[!ValueQ[cqmgGQMleafCurveIterationNullSTAT],cqmgGQMleafCurveIterationNullINIT[X,d],If[cqmgGQMleafCurveIterationNullSTAT!={X,d},cqmgGQMleafCurveIterationNullINIT[X,d],cqmgGQMleafCurveIterationNull]](*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
]];
If[vFix,(*prepares a function that fixes v if vFix==True and gives back the input if vFix==False*)
fx7[input_]:=v,
fx7[input_]:=input
...
cqmgTSleafCurveIterationAdaptiveNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgTSleafCurveIterationAdaptiveNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgTSleafCurveIterationAdaptiveNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgQMleafCurveIterationAdaptiveNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgQMleafCurveIterationAdaptiveNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgQMleafCurveIterationAdaptiveNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgGQMleafCurveIterationAdaptiveNullINIT[X_,d_]:=(
Xnum=N[X];(*ensure that X is numerical*)
Dim=Length[Xnum];(*=D*)
Nim=Length[Xnum[[1]]];(*=N*)
Id=IdentityMatrix[Nim]/2;(*prepare modified identity matrix for H*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgGQMleafCurveIterationAdaptiveNullSTAT={X,d};(*this variable indicates with which input qmgBasicOptimized has been compiled*)
...
d=cqmgGQMleafCurveIterationAdaptiveNullSTAT[[2]];(*=d*)
vPrime=out[[1]];
xNew=out[[2]];
{vPrime,xNew}
)
...
cqmgMinimizeLambda[X_,x_,d_,leaf_:"TSleaf",delta_:1,nMax_:100,epsilon_:10^-8]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
cqmgCurveIterationAdaptiveNull=If[leaf=="TSleaf",(*pick the appropriate function depending on the chosen leaf*)
If[!ValueQ[cqmgTSleafCurveIterationAdaptiveNullSTAT],cqmgTSleafCurveIterationAdaptiveNullINIT[X,d],If[cqmgTSleafCurveIterationAdaptiveNullSTAT!={X,d},cqmgTSleafCurveIterationAdaptiveNullINIT[X,d],cqmgTSleafCurveIterationAdaptiveNull]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[leaf=="QMleaf",
If[!ValueQ[cqmgQMleafCurveIterationAdaptiveNullSTAT],cqmgQMleafCurveIterationAdaptiveNullINIT[X,d],If[cqmgQMleafCurveIterationAdaptiveNullSTAT!={X,d},cqmgQMleafCurveIterationAdaptiveNullINIT[X,d],cqmgQMleafCurveIterationAdaptiveNull]],(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[!ValueQ[cqmgGQMleafCurveIterationAdaptiveNullSTAT],cqmgGQMleafCurveIterationAdaptiveNullINIT[X,d],If[cqmgGQMleafCurveIterationAdaptiveNullSTAT!={X,d},cqmgGQMleafCurveIterationAdaptiveNullINIT[X,d],cqmgGQMleafCurveIterationAdaptiveNull]](*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
]];
...
cqmgScan[X_,x_,delta_,nPrime_,n_,m_,d_,leaf_:"TSleaf"]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
...
distNew=cqmgDistribution[X,xNew,d,leaf];
vNew=Transpose[distNew] . RandomPoint[Ball[ConstantArray[0,d],1]];(*randomly choos an initial v in the distribution, see [1] section 3.3.3; this is done with respect to an orthonormal basis*)
xssNew=cqmgCurveIntegration[X,xNew,vNew,delta,n,m,d,leaf,False];(*integrate a new curve corresponding to the initial data, see [1] section 3.3.3*)
...
cqmgCoordinates[X_,x_,delta_,n_,m_,d_,leaf_:"TSleaf",calculateJaccobians_:True,epsilon_:10^-8]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
dist=cqmgDistribution[X,x,d,leaf];(*calculate distribution at x*)
xss={x};(*initialize set of coordinate points*)
For[s=1,s<=d,s++,((*iterate over the directions*)
xssNewPlus=cqmgCurveIntegration[X,xss,dist[[s]],delta,n,m,d,leaf,True];(*parallely integrate curve in the direction s for all already calculated coordinate lines, see [1] section 3.3.4; here we use parallelization*)
xssNewMinus=cqmgCurveIntegration[X,xss,-dist[[s]],delta,n,m,d,leaf,True];(*parallely integrate curve in the -(direction s) for all already calculated coordinate lines, see [1] section 3.3.4; here we use parallelization*)
...
xssCoordinates=Transpose[ArrayReshape[xss,Join[Table[2*n+1,d],{Dim}]],Join[Table[d+1-s,{s,d}],{d+1}]];(*reshape in such a manner that the index structure fits to [1] section 3.3.4*)
If[calculateJaccobians,((*optionally calculate Jaccobians*)
xssDerivatives={};(*initialize set of derivatives*)
For[s=1,s<=d,s++,((*iterate over directions*)
xssNewPlus=cqmgCurveIntegration[X,xss,dist[[s]],epsilon*delta,1,0,d,leaf,True];(*calculate on infinitesimal step in direction s, see [1] section 3.3.4; here we use parallelization*)
...
xssJaccobians=Transpose[ArrayReshape[xssDerivatives,Join[{d},Table[2*n+1,d],{Dim}]],Join[{d+1},Table[d+1-c,{c,d}],{d+2}]];(*reshape in such a manner that the index structure fits to [1] section 3.3.4*)
...
cqmgIntegrateTile[X_,xCollection_,Tile_,delta_,n_,m_,d_,leaf_:"TSleaf",measure_:"omega",epsilon_:10^-8]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
...
If[!ValueQ[cqmgPointToolsSTAT],cqmgPointToolsINIT[X,d],If[cqmgPointToolsSTAT!={X,d},cqmgPointToolsINIT[X,d],{cqmgPointInTileQ,cqmgPointsInTile,cqmgFindOptimalPointInTile,cqmgFilledTileQ}]];(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
If[!ValueQ[cqmgIntegrandsSTAT],cqmgIntegrandsINIT[X,d],If[cqmgIntegrandsSTAT!={X,d},cqmgIntegrandsINIT[X,d],cqmgIntegrands]];(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
...
{xsCoordinates,xsJaccobians}=cqmgCoordinates[X,xOptimal,delta,n,m,d,leaf,True,epsilon];(*construct local coordinates, see [1] section 3.3.5*)
xsCoordinates=Flatten[xsCoordinates,d-1];(*reshape for better processability*)
xsJaccobians=Flatten[xsJaccobians,d-1];(*reshape for better processability*)
...
FilledTileQ=cqmgFilledTileQ[ArrayReshape[xsCoordinatesInTileQ,Table[2*n+1,d]],Tile];(*use the latter calculation to check if the given tile is filled by the constructed coordinates, see [1] section 3.3.5*)
...
cqmgIntegrateTiling[X_,xCollection_,Tiling_,delta_,n_,m_,d_,leaf_:"TSleaf",measure_:"omega",epsilon_:10^-8,parallelTable_:True]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
TilesNumber=Length[Tiling];(*calculate the number of given tiles*)
If[parallelTable,
out=ParallelTable[cqmgIntegrateTile[X,xCollection,Tiling[[alpha]],delta,n,m,d,leaf,measure,epsilon],{alpha,TilesNumber}],(*integrate over each given tile; here we use parallelization*)
out=Table[cqmgIntegrateTile[X,xCollection,Tiling[[alpha]],delta,n,m,d,leaf,measure,epsilon],{alpha,TilesNumber}](*integrate over each given tile*)
];
...
cqmgQuantization[X_,d_,outIntegrateTiling_]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
...
alpha=(2Pi)^(d/2)/Vol;(*calculate alpha, see [1] equation (133)*)
...
cqmgIntegrateTilePreview[X_,xCollection_,Tile_,delta_,n_,m_,d_,leaf_:"TSleaf"]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
If[!ValueQ[cqmgPointToolsSTAT],cqmgPointToolsINIT[X,d],If[cqmgPointToolsSTAT!={X,d},cqmgPointToolsINIT[X,d],{cqmgPointInTileQ,cqmgPointsInTile,cqmgFindOptimalPointInTile,cqmgFilledTileQ}]];(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
xsInTile=cqmgPointsInTile[xCollection,Tile];(*select the points of xs that lie within the given tile, see [1] section 3.3.5*)
...
TileNonempty=True;(*in this cas the given tile is not empty, see [1] section 3.3.5*)
xOptimal=cqmgFindOptimalPointInTile[xsInTile,Tile];(*find the optimally located point within the given tile, see [1] section 3.3.5*)
{xsCoordinates}=cqmgCoordinates[X,xOptimal,delta,n,m,d,leaf,False];(*construct local coordinates, see [1] section 3.3.5*)
xsCoordinates=Flatten[xsCoordinates,d-1];(*reshape for better processability*)
xsCoordinatesInTileQ=cqmgPointInTileQ[xsCoordinates,Tile];(*for each coordinate point check if it lies within the given tile, see [1] section 3.3.5; here we use parallelization*)
FilledTileQ=cqmgFilledTileQ[ArrayReshape[xsCoordinatesInTileQ,Table[2*n+1,d]],Tile];(*use the latter calculation to check if the given tile is filled by the constructed coordinates, see [1] section 3.3.5*)
xsCoordinatesInTile=Pick[xsCoordinates,xsCoordinatesInTileQ];(*we restrict ourselves to points within the given tile, see [1] section 3.3.5*)
...
cqmgIntegrateTilingPreview[X_,xCollection_,Tiling_,delta_,n_,m_,d_,leaf_:"TSleaf",measure_:"omega",epsilon_:10^-8,parallelTable_:True]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
TilesNumber=Length[Tiling];(*calculate the number of given tiles*)
If[parallelTable,
out=ParallelTable[cqmgIntegrateTilePreview[X,xCollection,Tiling[[alpha]],delta,n,m,d,leaf],{alpha,TilesNumber}],(*integrate over each given tile; here we use parallelization*)
out=Table[cqmgIntegrateTilePreview[X,xCollection,Tiling[[alpha]],delta,n,m,d,leaf],{alpha,TilesNumber}](*integrate over each given tile*)
];
xsCoordinatesCollection=Table[out[[alpha,2,1]],{alpha,TilesNumber}];(*collect all coordinates that we used*)
NoTileEmpty=AllTrue[Table[out[[alpha,3,1]],{alpha,TilesNumber}],#&];(*checks if no given tile is empty*)
AllPointsInTile=(Sum[out[[alpha,3,2]],{alpha,TilesNumber}]==Length[xCollection]);(*checks if ever point of xs belongs to some given tile*)
...
cqmgCustomIntegrateTiling[X_,d_,outIntegrateTiling_,fxCustom_,parallelTable_:True]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
out=cqmgQuantization[X,d,outIntegrateTiling];
...
Module[{md=d,mDim=Dim,mNim=Nim,mJ=J,a,b,CovDerReal,Vprelim,V,Vprime,c},(*create module for improved compilation*)
...
CompilationOptions->{"InlineExternalDefinitions"->True},
RuntimeAttributes->{Listable}];(*allow for parallelization*)
cqmgKaehlerCostSTAT={X,d};(*this variable indicates with which input cqmgLightweight has been compiled*)
cqmgKaehlerCost
)
(**)
...
If[!ValueQ[cqmgKaehlerCostSTAT],cqmgKaehlerCostINIT[X,d],If[cqmgKaehlerCostSTAT!={X,d},cqmgKaehlerCostINIT[X,d],cqmgKaehlerCost]];(*if not yet compiled compile, if compiled for different input compile, else reuse already compiled version*)
...
cqmgKaehlerForLeaf[X_,x_,d_,leaf_:"TSleaf"]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
dist=cqmgDistribution[X,x,d,leaf];(*calculate the distribution*)
c=cqmgKaehler[X,x,Transpose[dist]];(*calculate the K\[ADoubleDot]hler cost*)
c
)


cqmgKaehlerForRandom[X_,x_,d_,n_]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
RandomSubspaces=RandomReal[{-1,1},{n,Dim,d}];(*choose random subspaces*)
...
cqmgQuantizationValidation[X_,x_,d_,outIntegrateTiling_,n_,leaf_:"TSleaf",tolerance_:10^-8]:=(
Dim=Length[X];(*=D*)
Nim=Length[X[[1]]];(*=N*)
{NoTileEmpty,AllPointsInTile,AllTilesFilled}=outIntegrateTiling[[3]];
{{Vol,alpha,nCorr},{Completeness,xBoldQuantizationCorr}}=cqmgQuantization[X,d,outIntegrateTiling];
{theta,thetaInducedByomega}=cqmgComparePoissonStructures[X,x,tolerance];
cLeaf=cqmgKaehlerForLeaf[X,x,d,leaf];
{cRandom,cRandomDev,cMin,cMax}=cqmgKaehlerForRandom[X,x,d,n];
...
heading={"Name","D","N","d","x","|x|","SymVol","dComp","dxBold","nCorr","dPoisson","cLeaf","cRandom","cRandomDev","cMin","cMax","NoTileEmpty","AllPointsInTile","AllTilesFilled"};(*prepare heading for table*)
data={heading};(*initialize*)
For[i=1,i<=Length[Xxs],i++,((*loop over all sets*)
X=Xxs[[i,1]];(*extract X*)
x=Xxs[[i,2]];(*extract x*)
...
n=Xxs[[i,6]];
leaf=Xxs[[i,7]];
{Vol,dCompleteness,dxBoldQuantizationCorr,nCorr,dPoisson,cLeaf,cRandom,cRandomDev,cMin,cMax,NoTileEmpty,AllPointsInTile,AllTilesFilled}=cqmgQuantizationValidation[X,x,d,outIntegrateTiling,n,leaf,10^-8];
AppendTo[data,{Name,Dim,Nim,d,MatrixForm[{N[Round[x,0.01]]}],N[Norm[x]],Vol,dCompleteness,dxBoldQuantizationCorr,nCorr,dPoisson,cLeaf,cRandom,cRandomDev,cMin,cMax,NoTileEmpty,AllPointsInTile,AllTilesFilled}];(*add data to table*)
)];
